...
+ (UIImage *)sk_imageWithSize:(CGSize)size drawBlock:(void (^)(CGContextRef context))drawBlock {
...
- (UIImage *)sk_imageByRoundCornerRadius:(CGFloat)radius
    ...
    return [self sk_imageByRoundCornerRadius:radius
...
- (UIImage *)sk_imageByBlurRadius:(CGFloat)blurRadius
    ...
    if (!hasBlur && !hasSaturation) {
        return [self sk_mergeImageRef:imageRef tintColor:tintColor tintBlendMode:tintBlendMode maskImage:maskImage opaque:opaque];
    }
    ...
    if (hasNewFunc) {
        ...
        if (err != kvImageNoError) {
            NSLog(@"UIImage+SKAdd error: vImageBuffer_InitWithCGImage returned error code %zi for inputImage: %@", err, self);
            return nil;
        }
        err = vImageBuffer_Init(&scratch, effect.height, effect.width, format.bitsPerPixel, kvImageNoFlags);
        if (err != kvImageNoError) {
            NSLog(@"UIImage+SKAdd error: vImageBuffer_Init returned error code %zi for inputImage: %@", err, self);
            return nil;
        }
    } else {
    ...
    if (hasNewFunc) {
        ...
        effectCGImage = vImageCreateCGImageFromBuffer(input, &format, &sk_cleanupBuffer, NULL, kvImageNoAllocate, NULL);
        if (effectCGImage == NULL) {
            effectCGImage = vImageCreateCGImageFromBuffer(input, &format, NULL, NULL, kvImageNoFlags, NULL);
            free(input->data);
        }
        free(output->data);
        outputImage = [self sk_mergeImageRef:effectCGImage tintColor:tintColor tintBlendMode:tintBlendMode maskImage:maskImage opaque:opaque];
...
static void sk_cleanupBuffer(void *userData, void *buf_data) {
    free(buf_data);
}
// Helper function to add tint and mask.
- (UIImage *)sk_mergeImageRef:(CGImageRef)effectCGImage
                     tintColor:(UIColor *)tintColor
